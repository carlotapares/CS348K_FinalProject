...
MIN_PIXEL_DIST_FAST_SPEED = 5 # 5 px displacement per frame
...
class ConditionChecker:
  ...
  def check(self):
    if self.condition == Condition.DIRECTION_RIGHT:
        return self.__check_direction_right()
    elif self.condition == Condition.DIRECTION_LEFT:
      return self.__check_direction_left()
    elif self.condition == Condition.SPEED_FAST:
      return self.__check_speed_fast()
    elif self.condition == Condition.SPEED_SLOW:
      return self.__check_speed_slow()
    elif self.condition == Condition.PLAYER_BACK:
      return self.__check_player_back()
    elif self.condition == Condition.PLAYER_FRONT:
  ...
  def __check_speed_fast(self):
    ...
    for i in range(1, len(self.predictions_)):
      next_pos = self.predictions_[i].get_bbox()
      dist += distance.cdist(prev_pos[0:1], next_pos[0:1], 'euclidean')
...
class Predicate:
  ...
  def run(self) -> list[Batch]:
    ...
    while len(out) < self.num_batches_:
      start = self.time_between_batches_* self.FPS_ * len(out)
      ...
      for f in batch_frames:
        frames_front.append(get_prediction(self.dataset_, f, "front"))
        frames_back.append(get_prediction(self.dataset_, f, "back"))
      ...
      if frames_front:
        batch = Batch()
        for i, f in enumerate(frames_front):
          img = get_image_data(self.path_ + self.filename_, batch_frames[i])
      ...
      if frames_back:
        batch = Batch()
        for i, f in enumerate(frames_back):
          img = get_image_data(self.path_ + self.filename_, batch_frames[i])
          data, w, h = get_prediction_vis(f, img)
          frame = Frame(data, w, h)
          batch.add_frame(frame)
...
def get_dataset_subset(filename: str, tags: list[str], num_batches: int, batch_size: int) -> list[Batch]:
  ...
  for tag in tags:
    if Condition.exists(tag):
      conditions.append(Condition(tag))
    else:
